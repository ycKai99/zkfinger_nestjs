...
namespace zkfpPrototype.Helper
{
    public class SDKHelper 
    {
        ...
        void axCZKEM1_OnAttTransactionEx(string EnrollNumber, int IsInValid, int AttState, int VerifyMethod, int Year, int Month, int Day, int Hour, int Minute, int Second, int WorkCode)
        {
            ...
            bool ret = axCZKEM1.GetUserTmpExStr(GetMachineNumber(), EnrollNumber.ToString(), 0, out flag, out fingerprintTemplate, out TemplateLength);
            if (ret == true)
            {
                gRealEventListBox.Items.Add($"Fingerprint template {fingerprintTemplate}");
                fingerprintTemplate = ReturnTemplate();
